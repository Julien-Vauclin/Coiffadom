...
class Booking
{
    ...
    public static function createBooking(string $BOOKING_DATE, string $BOOKING_TIME, string $BOOKING_DURATION, string $BOOKING_USER_ID, string $BOOKING_TYPE_ID, string $BOOKING_COST, string $HAIR_LENGTH_ID)
    {
        try {
            $pdo = Database::createInstancePDO();
            $sql = "INSERT INTO `booking` (`BOOKING_DATE`, `BOOKING_TIME`, `BOOKING_DURATION`, `BOOKING_USER_ID`, `BOOKING_TYPE_ID`, `BOOKING_COST`, `HAIR_LENGTH_ID`) VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
