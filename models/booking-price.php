...
class Booking
{
    ...
    public static function createBooking(string $BOOKING_TYPE_ID, string $HAIR_LENGTH_ID, string $BOOKING_DURATION, string $BOOKING_PRICE)
    {
        try {
            $pdo = Database::createInstancePDO();
            $sql = "INSERT INTO `booking` (`BOOKING_TYPE_ID`, `HAIR_LENGTH_ID`, `BOOKING_DURATION`, `BOOKING_PRICE`) VALUES (?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
